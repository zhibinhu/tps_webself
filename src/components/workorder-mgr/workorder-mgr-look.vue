<template>
    <div>
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" v-show='code!=0'>查看</li>
            <li class="breadcrumb-item active" v-show='code==0'>新增</li>
        </ol>
        <form class="m-3" autocomplete="off" @submit.prevent="submit">
            <div class="card p-3 mb-3" style="margin-top: 10px">
                ...
                <div class="form-group row" >
                    <div class="col-md-1">
                    <label  style="width:87px;" v-show='code!=0'>工单编号</label>
                    </div>
                    <div class="col-md-3" v-show='code!=0'>
                        {{worList.id}}
                    </div>
                    ...
                    <label style="width:87px;" v-show='code!=0'>工单状态:</label>
                    <div class="col-md-3" v-show='code!=0'>
                        {{worList.orderStatusName}}
                    </div>
                </div>
                <!--<div class="form-group row">-->
                ...
                <div style="border:1px solid #99DCF3;margin:20px 0;" v-show='code!=0'>
                    ...
                    <div style="padding:0 30px;border-bottom:1px solid #b4dbed ;height:auto;margin-top: 20px"
                         v-for="(item, index) in it">
                        <div style="display: inline;margin-top: 5%;">
                            <img :src="item.avatar" style="position: absolute;border-radius:15px;border: none;"
                ...
                <div style="border:1px solid #99DCF3;">
                    ...
                    <p style="padding:0 30px;">
                        <!--<quill-editor style="height:300px;" ref="myTextEditor" v-model="replayContent" :config="editorOption" @blur="onEditorBlur($event)" @focus="onEditorFocus($event)" @ready="onEditorReady($event)">-->
                        <!--</quill-editor>-->
            ...
            <div class="px-3" style="margin-bottom: 40px;text-align: center;">
...
<script>
    ...
    export default {
        ...
        beforeRouteEnter(to, from, next) {
            ...
            let pageType = dataparams[0]
            if (pageType == 0) {
                let orderType = dataparams[1]
                let orderId = dataparams[2]
                let createUserId = dataparams[3]
                let acceptUserId = dataparams[4]
                let planTargetId = dataparams[5]
                Promise.all([Workorder.get(orderId)]).then((data) => {
                    next(vm => {
                        console.log(JSON.stringify(data,null,4),'===================')
                        vm.orderId = orderId;
                        vm.orderType = orderType;
                        vm.createUserId = createUserId;
                        vm.acceptUserId = acceptUserId;
                        vm.replayUserId = acceptUserId;
                        vm.planTargetId = planTargetId;
                        vm.code = data[0].data.code
                        let array = data[0].data.contentList
                        vm.worList = data[0].data.workOrder
            ...
            if (pageType != 0) {
                let replayUserId = dataparams[1]
                let planTargetId = dataparams[2]
        ...
        methods: {
            ...
            postworkorderContent() {
                ...
                Workorder.workordercontentPost({
                    ...
                    console.log(JSON.stringify(response.data, null,4),"+++++++++++++response++++++++++++")
