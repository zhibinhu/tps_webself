    ...
    loadDICOMPromise.then(function (dicomPart10AsArrayBuffer /* , xhr*/) {
      ...
      try {
        ...
        return reject(error);
			...
			if(!(dataSet.elements.x7fe00010 || dataSet.elements.x7fe00008)){
				return reject("图像数据获取错误");
			}
//			console.log(dataSet,"图像数据");
      loadedDataSets[uri] = {
        dataSet: dataSet,
...
var defaultConfig = {
  ...
  webWorkerPath: './public/cornerstoneWADOImageLoaderWebWorker.min.js',
  webWorkerTaskPaths: [],
  taskConfiguration: {
    decodeTask: {
...
var files = [];
var instanceUID_imageID = {};

function add(file,instanceUid) {
	
  if(instanceUID_imageID[instanceUid]){
  	var fileIndex = Number(instanceUID_imageID[instanceUid].split("dicomfile:")[1]);
//	console.log(fileIndex);
  	files[fileIndex] = file;
	//console.log(11111,instanceUID_imageID[instanceUid]);
  	return instanceUID_imageID[instanceUid];
  }else{
  	var fileIndex = files.push(file);
  	if(instanceUid){
  		instanceUID_imageID[instanceUid] = "dicomfile:" + (fileIndex - 1);  		
  	}
	//console.log(11111,instanceUID_imageID[instanceUid]);
...
function metaDataProvider(type, imageId) {
  ...
  if (type === 'imagePlaneModule') {
	...
	// imagerPixelSpacing
	var magnification = dataSet.floatString('x00181114');
	var sourceTOdetector = dataSet.floatString('x00181110');
	var sourceTOpatient = dataSet.floatString('x00181111');
	var yPSnum = NaN;
	var xPSnum = NaN;
	var xyPSnum = [];
	var imagerPixelSpacing_now = [];
//	console.log('放大率',pixelSpacing,magnification,sourceTOpatient,sourceTOdetector)
	if(pixelSpacing) {
		xPSnum = pixelSpacing[0];
		yPSnum = pixelSpacing[1];
	} else if(imagerPixelSpacing) {
		// 判断放大率是否存在
		if(!magnification) {
			magnification = sourceTOpatient / sourceTOdetector;
			// 判断重新计算之后的放大率是否合法
			if(magnification >= 0.00001){
				xPSnum = imagerPixelSpacing[0] / magnification;
				yPSnum = imagerPixelSpacing[1] / magnification;
			}else{
				imagerPixelSpacing_now = [];
			}
		} else {
			// 放大率直接用
			xPSnum = imagerPixelSpacing[0] / magnification;
	...
	imagerPixelSpacing_now.push(xPSnum);
	imagerPixelSpacing_now.push(yPSnum);
//	console.log(11111,imagerPixelSpacing_now)
		
    var columnPixelSpacing = 1.0;
    var rowPixelSpacing = 1.0;

    rowPixelSpacing = xPSnum;
    columnPixelSpacing = yPSnum;
//	console.log(pixelSpacing,imagerPixelSpacing,12345678911);
    var rowCosines = null;
    ...
    return {
      ...
      pixelSpacing: imagerPixelSpacing_now,
